...
from comparison import error_future
...
v_new = p(t_new)
tshow=pd.DataFrame(data, columns=['Tfuture_confirmed'])
ts=tshow.Tfuture_confirmed

vreal=pd.DataFrame(data, columns=['Real_value_future'])
vr=vreal.Real_value_future
vgot=vreal=pd.DataFrame(data, columns=['Value_future_got'])
vg=vgot.Value_future_got
plt.figure(figsize = (9,3))

error_future(ts,p,vg ,vr, plt, np,'.')
